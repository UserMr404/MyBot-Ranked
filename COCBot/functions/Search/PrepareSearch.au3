...
; Example .......: No
; ===============================================================================================================================
Func PrepareSearch($Mode = $DB, $bRankedMode = False) ;Click attack button and find match button, will break shield

	; Set attack mode flag
	$g_bCurrentAttackIsRanked = $bRankedMode

	SetLog($bRankedMode ? "Going to Ranked Attack (" & ($g_iRankedAttacksToday + 1) & "/8)" : "Going to Normal Attack", $COLOR_INFO)
	...
	If IsMainPage() Then
		...
		; Check for ranked mode restrictions first
		If $bRankedMode Then
			; Check daily limit for ranked attacks
			If $g_iRankedAttacksToday >= $g_iMaxRankedAttacks Then
				SetLog("Ranked attack limit reached for today (" & $g_iRankedAttacksToday & "/8)", $COLOR_ERROR)
				Return
			EndIf
			SetLog("Ranked Mode: Will attack first base found (no search)", $COLOR_WARNING)
		EndIf

		; Detect and click appropriate attack button
		Local $bButtonFound = False
		Local $AttackButtonClickXY[2]
		
		If $bRankedMode Then
			; Try to find Ranked Attack button
			Local $aRankedAttack = findButton("RankedAttackButton", Default, 1, True)
			If IsArray($aRankedAttack) And UBound($aRankedAttack, 1) = 2 Then
				$AttackButtonClickXY[0] = $aRankedAttack[0]
				$AttackButtonClickXY[1] = $aRankedAttack[1]
				$bButtonFound = True
				SetLog("Found Ranked Attack button", $COLOR_SUCCESS)
			Else
				; Fallback to normal attack button for ranked mode
				Local $aNormalAttack = findButton("AttackButton", Default, 1, True)
				If IsArray($aNormalAttack) And UBound($aNormalAttack, 1) = 2 Then
					$AttackButtonClickXY[0] = $aNormalAttack[0]
					$AttackButtonClickXY[1] = $aNormalAttack[1]
					$bButtonFound = True
					SetLog("Using Normal Attack button for Ranked mode", $COLOR_INFO)
				EndIf
			EndIf
		Else
			; Try to find Normal Attack button
			Local $aNormalAttack = findButton("NormalAttackButton", Default, 1, True)
			If IsArray($aNormalAttack) And UBound($aNormalAttack, 1) = 2 Then
				$AttackButtonClickXY[0] = $aNormalAttack[0]
				$AttackButtonClickXY[1] = $aNormalAttack[1]
				$bButtonFound = True
				SetLog("Found Normal Attack button", $COLOR_SUCCESS)
			Else
				; Fallback to legacy attack button detection
				...
				If IsArray($aAttack) And UBound($aAttack, 1) = 2 Then
					...
					$AttackButtonClickXY[0] = Random($AttackCoordsX[0], $AttackCoordsX[1], 1)
					$AttackButtonClickXY[1] = Random($AttackCoordsY[0], $AttackCoordsY[1], 1)
					$bButtonFound = True
					SetLog("Using legacy Attack button detection", $COLOR_INFO)
				Else
					; Try rescue attack button
					...
					If IsArray($aRescueAttack) And UBound($aRescueAttack, 1) = 2 Then
						...
						$AttackButtonClickXY[1] = Random($AttackCoordsY[0], $AttackCoordsY[1], 1)
						$bButtonFound = True
						SetLog("Using Rescue Attack button", $COLOR_INFO)
					EndIf
				EndIf
			EndIf
		EndIf

		If $bButtonFound Then
			ClickP($AttackButtonClickXY, 1, 130, "#0149")
		Else
			SetLog("Couldn't find " & ($bRankedMode ? "Ranked" : "Normal") & " Attack Button!", $COLOR_ERROR)
			If $g_bDebugImageSave Then SaveDebugImage("AttackButtonNotFound_" & ($bRankedMode ? "Ranked" : "Normal"))
			Return
		EndIf
	EndIf
