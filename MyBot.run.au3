...
Func InitializeBot()
	...
	If $bConfigRead Or FileExists($g_sProfileBuildingPath) Then
		readConfig()
		; Load ranked attack configuration
		LoadRankedConfig()
		SetLog("Ranked attack system initialized", $COLOR_INFO)
	EndIf
...
Func runBot() ;Bot that runs everything in order
	...
	; Initialize Ranked Attack System
	CheckRankedDailyReset()
	If $g_bRankedModeEnabled Then
		SetLog("Ranked Attack Mode is enabled - " & ($g_iMaxRankedAttacks - $g_iRankedAttacksToday) & " attacks remaining today", $COLOR_INFO)
	EndIf
...
Func AttackMain() ;Main control for attack functions
	...
	If IsSearchAttackEnabled() Then
		If IsSearchModeActive($DB) Or IsSearchModeActive($LB) Then
			...
			; Normal attack system (existing logic)
			PrepareSearch()
			If Not $g_bRunState Then Return
